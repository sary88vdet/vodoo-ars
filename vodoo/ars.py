...
def process_payment(debt_previous_month, invoice, payment, payment_in_adv=0):
    ...
    if debt_previous_month == 0:
        ...
        if payment <= invoice:
            debt_after_payment = invoice - payment
            current_month = payment
            payment_in_adv += 0
        else:
            payment_in_adv += payment - invoice
    ...
    # If with debt
        ...
        if payment < total_debt:
            current_month_debt = payment
            debt_after_payment = invoice + (total_debt - payment)
            current_month = 0
            payment_in_adv += 0
        else: #payment is larger than total_debt
            current_month_debt = total_debt
            current_month = payment - total_debt
